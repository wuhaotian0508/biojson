...
# RAGSearchTool 需要组合多个子组件：
# - pubmed_tool: 在线检索 PubMed 文献
# - retriever: 本地向量库检索
# - reranker: 对混合检索结果重排序
# - get_personal_lib=None: 测试环境无需个人文献库
# - get_query_embedding: 复用 retriever 的 embedding 函数，避免重复初始化模型
rag_search_tool = RAGSearchTool(
    pubmed_tool=pubmed_tool,
    retriever=retriever,
    reranker=reranker,
    get_personal_lib=lambda uid: None,  # 测试环境不使用个人库
