...
from search.query_translator import configure_llm as configure_query_translator
from tools import Toolregistry
from tools.pubmed_search import PubmedSearchTool, configure_query_optimizer
...
gene_db_tool = GeneDBSearchTool(retriever)
...
rag_search_tool = RAGSearchTool(
    sources={
        ...
        "personal_lib": personal_tool,
    },
    reranker=reranker,
)

registry.register(pubmed_tool)                              # PubMed（单独搜索）
registry.register(gene_db_tool)                             # 基因数据库（单独搜索）
...
async def admin_reindex_status(user=Depends(get_current_user)):
    ...
    # 统计 DATA_DIR 中的文件数
    data_files = list(config.DATA_DIR.glob("*_nutri_plant_verified.json"))
