...
│   └── user_skills/        # 用户自定义技能
├── extractor/              # Markdown -> 抽取 JSON -> verified JSON 流水线
│   ├── input/              # 待处理 Markdown
...
```bash
MD_DIR=./extractor/input
JSON_DIR=./data/corpus
REPORTS_DIR=./extractor/reports
TOKEN_USAGE_DIR=./extractor/reports/token-usage
PROCESSED_DIR=./extractor/input/processed
...
```bash
uv run uvicorn server.web:app --host 0.0.0.0 --port 5000
```

## 单独启动 Admin

Admin 也可以作为独立 Flask 应用运行：

```bash
uv run nutrimaster admin
```

默认端口为 `5501`，可通过 `ADMIN_PORT` 配置。

Admin 功能包括上传论文 ZIP、去重、单篇预览、批量抽取与验证、SSE 进度、prompt/schema 编辑、索引重建和已处理论文列表。Admin 访问由 `ADMIN_EMAIL` 白名单控制。

## 启动 Headless API

Headless API 去掉完整 Web UI，保留核心 RAG API：

```bash
uv run nutrimaster api --host 0.0.0.0 --port 8000
```
...
Extractor 从 `extractor/input/` 读取 Markdown，输出 verified JSON 到 `data/corpus/`。
...
5. 验证报告和 token 报告写入 `extractor/reports/`。

注意：`extractor/config.py` 在 import 时读取环境变量。Admin 因此会在 import extractor 前先设置 `JSON_DIR`、`PROMPT_PATH` 和 `SCHEMA_PATH`。
...
- `extractor/input/processed/`、`extractor/output/`、token 报告、个人库、用户技能、日志和索引二进制产物默认不应提交。
- Admin 和 `extractor/run.sh` 默认使用 v5 prompt/schema；直接 import `extractor.config` 时会使用该文件自身默认值，必要时用环境变量显式覆盖。
...
4. 将 Markdown 论文放入 `extractor/input/`。
