...
def test_stack_defaults_use_canonical_jina_retriever():
    ...
    source = inspect.getsource(stack_module._legacy_defaults)

    assert "search.retriever" not in source
    assert "retrieval.jina_retriever" in source


def test_legacy_search_retriever_facade_is_removed():
    canonical_retriever = importlib.import_module("retrieval.jina_retriever")
