...
from agent.tools import CrisprTool, ReadTool, ShellTool, WriteTool
...
from retrieval.query_translation import configure_llm as configure_query_translator
from shared.llm import call_llm, call_llm_stream
from shared.settings import Settings


@dataclass(frozen=True)
class LegacyAgentStack:
    retriever: Any
    registry: Any
    skill_loader: Any
...
def build_legacy_agent_stack(
    *,
    ...
    """Build the current RAG Agent stack behind a stable NutriMaster boundary."""

    needs_legacy_defaults = any(
        item is None
        for item in (
            retriever,
            registry_factory,
    ...
    if needs_legacy_defaults:
        ...
        configure_query_translator = (
            configure_query_translator or defaults["configure_query_translator"]
        )
        configure_query_optimizer = (
            configure_query_optimizer or defaults["configure_query_optimizer"]
        )
    ...
    return LegacyAgentStack(
        retriever=retriever,
        registry=registry,
        skill_loader=skill_loader,
        agent_runtime=AgentRuntime(legacy_agent),
        tools=tools,
    )
