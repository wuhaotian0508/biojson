 ...
 *
 *  模块：
 *    1. Auth      — Supabase 邮箱/密码认证（复用 rag/web 的模式）
 ...
 *    6. Papers    — 表格展示 rag/data/ 中所有 verified JSON
...
// API 基础路径（集成到 rag web app 时为 /admin，独立运行时为空）
...
function renderUploadResult(data) {
    ...
    if (data.skipped_processed.length) {
        html += `<div style="margin-top:10px"><span class="count-skip">${data.skipped_processed.length}</span> skipped (already in rag/data)</div>`;
 ...
 * 自动跳过 rag/data 中已有 verified JSON 的文件。
 */
async function startRun() {
    try {
        ...
        // 如果有跳过的文件，显示提示
        if (data.skipped > 0) {
            addLogLine(`Skipped ${data.skipped} papers (already in rag/data)`, 'info');
        }
