"""
admin/app.py — BioJSON Admin Panel 后端（Flask, 默认端口 5501）
...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  环境变量设置 — 必须在 import extractor 之前完成                            ║
# ║  因为 extractor/config.py 在 import 时就读取 env，之后无法再改              ║
# ╚══════════════════════════════════════════════════════════════════════════╝

REPO_ROOT = Path(__file__).resolve().parent.parent

# 设置 extractor 需要的关键路径（setdefault 不会覆盖已存在的值）
os.environ.setdefault("JSON_DIR", str(REPO_ROOT / "rag" / "data"))
os.environ.setdefault("PROMPT_PATH", str(REPO_ROOT / "extractor" / "prompts" / "nutri_gene_prompt_v5.txt"))
os.environ.setdefault("SCHEMA_PATH", str(REPO_ROOT / "extractor" / "prompts" / "nutri_gene_schema_v5.json"))
os.environ.setdefault("MD_DIR", str(REPO_ROOT / "extractor" / "input"))
...
from extractor.config import INPUT_DIR, ensure_dirs
from extractor.pipeline import process_one_paper, run_pipeline_batch, save_token_report
...
# Blueprint 版本（供 rag/web/app.py 集成使用）
...
# 独立运行时的 Flask app（python -m admin.app）
app = Flask(__name__, static_folder="static", static_url_path="/static")

# 下方所有路由都注册到 admin_bp 上
# standalone app 在 __main__ 中通过 app.register_blueprint(admin_bp) 挂载
...
# 常用路径
DATA_DIR = REPO_ROOT / "rag" / "data"            # verified JSON 最终存放目录
...
def get_processed_stems() -> set:
    """扫描 rag/data/ 获取已处理文件的 stem 集合（用于上传去重）。
...
def get_input_files() -> list:
    """获取 extractor/input/ 中所有待处理的 .md 文件名（排序后）。"""
...
def _apply_settings(settings: dict):
    ...
    import extractor.verify as _ver
...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  路由：ZIP 上传                                                           ║
# ║  递归解压所有嵌套 zip → 提取 .md → 去重后存入 extractor/input/               ║
# ╚══════════════════════════════════════════════════════════════════════════╝
...
def api_upload():
    ...
    去重规则：
      1. stem 已在 rag/data/ 中有 verified JSON → 跳过（"已在库中"）
      2. stem 已在 extractor/input/ 中 → 跳过（"已在队列中"）
    """
    ...
    skipped_processed = []  # 跳过：rag/data 中已有
...
def api_pipeline_preview():
    ...
    预览是真实处理——.md 会移入 processed，JSON 写入 rag/data/。
...
def api_pipeline_run():
    ...
    # ── 过滤掉 rag/data 中已有 verified JSON 的文件（只看 data 目录，不管 report）──
    processed_stems = get_processed_stems()
    ...
    if not todo_files:
        ...
        return jsonify({"error": f"全部 {len(files)} 篇已在 rag/data 中，无需处理"}), 400
    ...
    def run():
        ...
        try:
            ...
            if skipped > 0:
                print(f"  ⏭️  跳过 {skipped} 篇（rag/data 中已有 verified JSON）")
            ...
            if not stopped:
                ...
                try:
                    import sys as _sys
                    _sys.path.insert(0, str(REPO_ROOT / "rag"))
...
@admin_required
def api_papers():
    """扫描 rag/data/ 返回所有已处理论文的列表（文件名、标题、修改时间）。"""
    papers = []
...
def api_prompt_put():
    ...
    try:
        from extractor.extract import _load_prompt
...
def api_schema_put():
    ...
    try:
        from extractor.extract import _load_extract_all_schema
...
if __name__ == "__main__":
    ...
    # 独立运行时，把 blueprint 注册到 standalone app（无前缀）
    app.register_blueprint(admin_bp)
    print(f"Admin panel: http://0.0.0.0:5501")
